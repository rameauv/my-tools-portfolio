 ...
 * 
 * Fetches repositories from a GitHub user and generates a TypeScript file
...
async function getOAuthToken() {
  ...
  const { deviceCode, userCode, verificationUri, expiresIn, interval } = await initiateOAuthDeviceFlow();
...
async function main() {
  try {
    ...
    if (!config.username) {
      ...
      console.error('Usage: node scripts/fetch-github-repos.mjs <username> [--output <path>] [--token <token>] [--oauth]');
      process.exit(1);
    }
    ...
    const fileContent = generateTypeScriptFile(reposWithReadme);
